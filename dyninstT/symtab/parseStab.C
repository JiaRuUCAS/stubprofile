		...
		switch (stabstr[cnt]) {
			...
			case 'F':/* Global Function */
			{
				...
				if (!mod->exec()->findFunctionsByName(fpv, symt_current_mangled_func_name)) {
					...
					if (NULL == (fp = mangledNameMatchKLUDGE(
													symt_current_func_name.c_str(),
													symt_current_mangled_func_name.c_str(),
													mod)))
						break;
				}
				fp = fpv[0];
			...
			case 'G':/* Global Varaible */
				...
				if (BPtype) {
					...
					if (!result) {
						...
						if (mod->exec()->getDefaultModule()
										->findVariablesByName(ret, name))
					...
					typeCollection *tc_to_use =
							typeCollection::getModTypeCollection(toUse);
			...
			case 'c': /* constant */
			{
				...
				if (symt_current_mangled_func_name.length()) {
					std::vector<Function *>fpv;
  					...
  					if (mod->exec()->findFunctionsByName(
											fpv, symt_current_mangled_func_name)) { 
						 ...
						 const char *name, int ID, unsigned int sizeHint)
{
	...
	if (isSymId(stabstr[0])) {
		...
		case 'g':  
		{
			...
			std::string tName = convertCharToString(name);
			typeFunction *newFunction = new typeFunction(ID, ptrType, tName);

...
typeFunction *typeFunction::create(std::string &name, Type *retType,
				tbb::concurrent_vector<Type *> &paramTypes,
				Symtab *obj)
{
...
typeSubrange *typeSubrange::create(std::string &name, int size, long low, long hi,
				Symtab *obj)
{
...
typeArray *typeArray::create(std::string &name, Type *type, long low, long hi,
				Symtab *obj)
{
...
typeStruct *typeStruct::create(std::string &name,
				tbb::concurrent_vector< std::pair<std::string, Type *> *> &flds,
				Symtab *obj)
{
	int offset = 0;
...
typeStruct *typeStruct::create(std::string &name,
				tbb::concurrent_vector<Field *> &flds,
				Symtab *obj)
{
	typeStruct *typ = new typeStruct(name);
	for(unsigned i=0;i<flds.size();i++)
...
typeUnion *typeUnion::create(std::string &name,
				tbb::concurrent_vector< std::pair<std::string, Type *> *> &flds,
				Symtab *obj)
{

...
#if !defined(__SYMTAB_OBJECT_ELF_H__)
#define __SYMTAB_OBJECT_ELF_H__
...
/*
 * The standard symbol table in an elf file is the .symtab section. This
 * section does not have information to find the module to which a global
 * symbol belongs, so we must also read the .stab section to get this info.
 *
 * .stab section contains an array of fixed length structures, one structure
 * per stab. .stabstr section contains all the variable length strings that
 * are referenced by stabs in .stab section.
 */
...
struct stab32 {
	...
	unsigned char other;
	unsigned short desc;
	/* value of this symbol -- usually zero. The real value must be obtained
	 * from the symtab section.
	 */
	unsigned int val;
};
struct stab64 {
...
#endif /* !defined(__SYMTAB_OBJECT_ELF_H__) */

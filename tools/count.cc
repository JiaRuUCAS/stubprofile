...
bool CountUtil::getTargetFuncs(void)
{
	...
	if (target_funcs.size() == 0) {
		LOG_ERROR("Failed to find functions %s", pattern.c_str());
		return false;
	}

	LOG_INFO("Found %lu functions for %s",
				...
				FuncMap *fmap, string pattern)
{
	...
	for (unsigned j = 0; j < tfuncs.size(); j++) {
		...
		LOG_INFO("Edit function %s:%s, ID %u",
						obj->name().c_str(),
...
#define DYNINSTLIB_PREFIX "/home/jr/stubprofile/build/lib/"
...
bool CountUtil::insertCount(void)
{
	...
	for (unsigned i = 0; i < target_funcs.size(); i++) {
		...
		LOG_INFO("Insert func_pre into %s",
						tf->func->getName().c_str());
		handle = as->insertSnippet(
						pre_expr, *pentry, BPatch_callBefore);
		if (!handle) {
			LOG_ERROR("Failed to insert func_pre to %s",
							tf->func->getName().c_str());
			return false;
		}

		LOG_INFO("Insert func_post into %s",
						tf->func->getName().c_str());
		handle = as->insertSnippet(
...
bool CountUtil::loadFunctions(void)
{
	...
	// load lib
	LOG_INFO("Load %s", LIBCNT);
	libcnt = as->loadLibrary(LIBCNT);
	if (!libcnt) {
		LOG_ERROR("Failed to load %s", LIBCNT);
		return false;
	}
...
bool CountUtil::insertExit(string filter)
{
	...
	for (unsigned int i = 0; i < funcs.size(); i++) {
		...
		if (!handle) {
			LOG_ERROR("Failed to insert exit function to %s",
							mod->getObject()->name().c_str());
			return false;
		}
	...
	if (!exit_point) {
		LOG_ERROR("Failed to find point for pthread_exit");
		return true;
	}
...
bool CountUtil::parseOption(int opt, char *optarg)
{
	switch(opt) {
		...
		case 'F':
			...
			if (!freq) {
				LOG_ERROR("Failed to parse sample frequency %s",
								optarg);
				return false;
			}
			break;

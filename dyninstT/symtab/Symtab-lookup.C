...
std::vector<Symbol *> Symtab::findSymbolByOffset(Offset o)
{
	...
	indexed_symbols::index<offset>::type& syms_by_offset =
			everyDefinedSymbol.get<offset>();
						...
						bool isRegex, bool checkCase, bool includeUndefined)
{
	...
	if (!isRegex) {
		...
		if (nameType & mangledName) {
			...
			if(includeUndefined) {
				std::copy(undefMangledSyms.equal_range(name).first,
								undefMangledSyms.equal_range(name).second,
								std::back_inserter(candidates));
			}
		}
		...
		if (nameType & prettyName) {
			...
			std::copy(pretty_range.first, pretty_range.second,
							std::back_inserter(candidates));
			if(includeUndefined) {
				std::copy(undefPrettySyms.equal_range(name).first,
								undefPrettySyms.equal_range(name).second,
								std::back_inserter(candidates));
			}
		}

		if (nameType & typedName) {
			std::copy(typedSyms.equal_range(name).first,
							typedSyms.equal_range(name).second,
							std::back_inserter(candidates));
			if(includeUndefined) {
				std::copy(undefTypedSyms.equal_range(name).first,
								undefTypedSyms.equal_range(name).second,
								std::back_inserter(candidates));
			}
		}
	}
		...
		if (includeUndefined) {
			cerr << "Warning: regex search of undefined symbols is not supported"
					<< endl;
		}

		for (auto i = everyDefinedSymbol.begin();
						i != everyDefinedSymbol.end(); i++) {
			if (nameType & mangledName) {
...
bool Symtab::getAllSymbols(std::vector<Symbol *> &ret)
{
	...
	std::copy(everyDefinedSymbol.begin(),
					everyDefinedSymbol.end(), back_inserter(ret));
...
bool Symtab::getAllSymbolsByType(
				std::vector<Symbol *> &ret, Symbol::SymbolType sType)
{
	if (sType == Symbol::ST_UNKNOWN)
...
bool Symtab::findFunctionsByName(std::vector<Function *> &ret,
				const std::string name, NameType nameType,
				bool isRegex, bool checkCase) {
	std::vector<Symbol *> funcSyms;
	if (!findSymbol(funcSyms, name,
							Symbol::ST_FUNCTION,
							nameType, isRegex, checkCase)) {
		return false;
	}
	...
	for (unsigned i = 0; i < funcSyms.size(); i++) {
		...
		if (!funcSyms[i]->getFunction()) {
			...
			create_printf("%s[%d]:  WARNING:  %s is %s a function\n",
							...
							funcSyms[i]->isFunction() ? "" : "not");
			create_printf("%s[%d]:  WARNING:  %s is %s a variable\n",
							FILE__, __LINE__, name.c_str(),
							funcSyms[i]->isVariable() ? "" : "not");
			continue;
		}
...
bool Symtab::getAllModules(std::vector<Module *> &ret)
{
	if (indexed_modules.size() >0 ) {
		std::copy(indexed_modules.begin(),
						indexed_modules.end(), std::back_inserter(ret));
		return true;
	}
...
bool Symtab::findRegion(Region *&ret, const Offset addr, const unsigned long size)
{
	...
	for(unsigned index=0;index<regions_.size();index++) {
		if(regions_[index]->getMemOffset() == addr &&
						regions_[index]->getMemSize() == size) {
			if (ret) {
				assert((addr == 0) ||
...
Region *Symtab::findEnclosingRegion(const Offset where)
{
	...
	while (last >= first) {
		...
		else {/* where >= (cursec->getSecAddr() + cursec->getSecSize()) */
...
pattern_match( const char *p, const char *s, bool checkCase ) {
	...
	while ( true ) {
		// If at the end of the pattern, it matches if also at the end of
		// the string
		if( *p == '\0' )
			return ( *s == '\0' );
		...
		if( *p == MULTIPLE_WILDCARD_CHARACTER ) {
			...
			// Try to match the remaining pattern for each remaining
			// substring of s
			for(; *s != '\0'; ++s )
		...
		// If at the end of the string (and at this point, not of the
		// pattern), it fails
		if( *s == '\0' )
			return false;

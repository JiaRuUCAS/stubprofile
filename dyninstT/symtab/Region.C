...
Region::Region(unsigned regnum, std::string name, Offset diskOff,
					unsigned long diskSize, Offset memOff,
					unsigned long memSize, char *rawDataPtr,
					perm_t perms, RegionType regType, bool isLoadable,
					bool isThreadLocal, unsigned long memAlignment) :
	regNum_(regnum), name_(name), diskOff_(diskOff),
	diskSize_(diskSize), memOff_(memOff),
	memSize_(memSize), fileOff_(0),
...
#if !defined(SERIALIZATION_DISABLED)
Serializable * Region::serialize_impl(
				SerializerBase *sb, const char *tag)
...
bool Region::addRelocationEntry(Offset ra, Symbol *dynref,
				unsigned long relType, Region::RegionType rtype)
{
	rels_.push_back(relocationEntry(
							ra, dynref->getMangledName(),
							dynref, relType, rtype));
	return true;
}

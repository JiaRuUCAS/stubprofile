...
{
	std::string modname = a.getModule() ? a.getModule()->fullName() : std::string("no_mod");
...
Aggregate::name_iter Aggregate::mangled_names_begin() const
{
	return boost::make_transform_iterator(
					symbols_.begin(), std::mem_fun(&Symbol::getMangledName));
}

...
//	friend class Serializable;
	...
	protected:
		...
		AnnotationClassBase(std::string n, anno_cmp_func_t cmp_func_ = NULL);
...
class COMMON_EXPORT AnnotatableDense
{
	...
	private:
		...
		bool addAnnotation(const void *a, AnnotationClassID id) {
			...
			/* Can have case where we have allocated annotations struct but
			 * not the actual annotations data array in case where we have
			 * performed serialization
			 */
			if (annotations->data == NULL) {
				annotations->data = (anno_list_t *)calloc(
								sizeof(anno_list_t), (size));
				...
				annotations->max = size;
				annotations->data = (anno_list_t *)realloc(
								annotations->data, sizeof(anno_list_t) * size);
			...
			if (annotations && annotations->data) {
				for (unsigned int i = 0; i < annotations->max; ++i) {
					if (NULL != annotations->data[i]) {
						AnnotationClassBase *acb =
								AnnotationClassBase::findAnnotationClass(i);
...
class COMMON_EXPORT AnnotatableSparse
{
	...
	private:
		...
		template <class T>
		AN_INLINE annos_by_type_t *getAnnosOfType(AnnotationClass<T> &a_id,
						bool do_create =false) const
		{
		...
		AN_INLINE bool addAnnotation(const void *a, AnnotationClassID aid)
		{
			...
			if (iter == abt->end())
				...
				/* do silent replacement -- this case can arise if an annotatable
				 * object is destroyed and then reallocated as a new object at
				 * the same address. Given the sparse map data structure, it
				 * would be rather expensive to go through the map in the dtor to
				 * eliminate references to the destroyed object.
				 *
				 * Added annotation removal in dtor...  so this case should NOT
				 * arise, do the replacement, but make some noise:
				 */

				if (a != iter->second) {
					annotatable_printf("%s[%d]:  WEIRD:  already have annotation"
									" of this type: %p, replacing with %p\n",
	...
	public:
		...
		AN_INLINE bool addAnnotation(const T *a, AnnotationClass<T> &a_id)
		{
			...
			if (iter == abt->end())
				 ...
				 * object is destroyed and then reallocated as a new object at
				 * the same address. Given the sparse map data structure, it
				 * would be rather expensive to  go through the map in the dtor
				 * to eliminate references to the destroyed object.
				 *
				 * Added annotation removal in dtor...  so this case should NOT
				 * arise, do the replacement, but make some noise:
				 */
		...
		void annotationsReport()
		{
			...
			for (AnnotationClassID id = 0; id < l_annos.size(); ++id) {
				...
				AnnotationClassBase *acb =
						AnnotationClassBase::findAnnotationClass(id);
				if (!acb) {
					fprintf(stderr, "%s[%d]:  FIXME, cant find anno class"
									" base for id %d\n",
									FILE__, __LINE__, id);
					continue;
				}

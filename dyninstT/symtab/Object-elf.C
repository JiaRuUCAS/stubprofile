...
struct SectionHeaderSortFunction :
		public binary_function<Elf_X_Shdr *, Elf_X_Shdr *, bool> {
...
/*
 * parseCompilerType
 * parse for compiler that was used to generate object.
...
void Object::parseStabTypes()
{
	...
	// Using the Object to get the pointers to the .stab and .stabstr
	// - Elf32 specific needs to be in seperate file -- jkh 3/18/99
	next_stabstr = stabptr->getStringBase();
	...
	for (i = 0; i < stabptr->count(); i++) {
		switch (stabptr->type(i)) {
			...
			case N_UNDF:
				/* value contains offset of the next string table for
				 * next module */
				stabptr->setStringBase(next_stabstr);
				next_stabstr = stabptr->getStringBase() + stabptr->val(i);
				break;
			...
			/* compliation source or file name */
			case N_SO:
				// reset for next object file
				symt_current_func_name = "";
				symt_current_mangled_func_name = "";
				...
				if (associated_symtab->findModuleByName(mod, modName)) {
					...
					mod = associated_symtab->getDefaultModule();
					tc = typeCollection::getModTypeCollection(mod);
					types_printf("\t Warning: failed to find module name "
								"matching %s, using %s\n",
		...
		if (parseActive || !is_aout()) {
			...
			switch (stabptr->type(i)) {
				case N_FUN:
					/* all we have to do with function stabs at this point
					 * is to assure that we have properly set the var
					 * currentFunctionName for the later case of (parseActive)
					 */
					symt_current_func = NULL;
					int currentEntry = i;
					...
					if (!ptr || !(colonPtr = strchr(ptr, ':')))
						...
						if (!associated_symtab->findSymbol(syms,
							...
							if (!associated_symtab->findSymbol(syms,
										...
										Symbol::ST_FUNCTION,
										mangledName)) {
								string fortranName = *currentFunctionName
										+ string("_");
			...
			switch (stabptr->type(i)) {
				case N_BCOMM: {
					...
					if (commonBlockVar) {
						commonBlock = dynamic_cast<typeCommon *>(
										tc->findVariableType(*commonBlockName));
						if (commonBlock == NULL) {
				...
				case 0xc8: // position-independant external typedefs -- N_ESYM
					...
					temp = parseStabString(mod, stabptr->desc(i),
